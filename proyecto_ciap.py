...
def objective(trial):
    ...
    xgb_model_v1 = XGBClassifier(
        ...
        reg_alpha=reg_alpha,
        scale_pos_weight=len(y_train[y_train==0]) / len(y_train[y_train==1]), # Usa y_train_res
    ...
    # Evaluar con validación cruzada utilizando X_train_res y y_train_res
    # Asegúrate de usar los datos balanceados (X_train_res, y_train_res) para el entrenamiento interno de CV
...
"""2. Nuevos ejemplos de transacciones"""
...
if loaded_model:
    ...
    # Generar ejemplos sintéticos de NO FRAUDE
    print(f"Generando {num_non_fraud_examples} ejemplos sintéticos de NO FRAUDE...")
    ...
    print(f"Generando {num_fraud_examples} ejemplos sintéticos de FRAUDE...")
    for _ in range(num_fraud_examples):
        new_row_dict = {}
        for col in feature_columns:
            mean_val = df_fraud[col].mean()
            std_val = df_fraud[col].std()
            # Generamos valores aleatorios alrededor de la media de transacciones fraudulentas
            # Para fraude, usamos 1.0*std para capturar una mayor dispersión
